...
class ImageOperations:
    ...
    def __init__(self,frame):
        self.frame = frame
        #Camera Parameters
        self.K =  np.asarray([[ 1.15422732e+03, 0.00000000e+00, 6.71627794e+02],
                            [0.00000000e+00, 1.14818221e+03, 3.86046312e+02],
                            [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.dist = np.asarray([[-2.42565104e-01, -4.77893070e-02, -1.31388084e-03, -8.79107779e-05, 2.20573263e-02]])
        ...
        self.src = np.array([[200, 720], [540, 480], [800, 480],[1200, 720]], np.float32)
        self.dst = np.array([[300, 720], [300, 0], [980, 0], [980, 720]], np.float32)
    ...
    def color_filter(self,image):
        ...
        lower_yellow = np.array([0, 30, 30], dtype="uint8")
...
def main():
    ...
    cap = cv2.VideoCapture('./input/challenge_video.mp4')
    

    codec = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
    out = cv2.VideoWriter('problem2b.avi',codec, 20.0, (1280,720))
    ...
    while(cap.isOpened()):
        ...
        if not ret:
            ...
            #pts = image.src
            #cv2.polylines(undistorted_img,[np.int32([pts])],True,(0,255,255))
            ...
            # sharpened_binary_image = image.sharpen(binary_unwarp,image.sharpening_kernel)
            # #debug.collect(sharpened_binary_image,("image","color"))

            unwarped_image= image.warpPerspective(sharpened_image,image.H)
            #debug.collect(unwarped_image,("image","color"))
            
            # lane_point_count = np.count_nonzero(binary_unwarp == 255, axis=0)
            # pts = [(x,lane_point_count[x]) for x in range(1280)]
            # #debug.collect(pts,("plot","gray"))
            ...
            randomlist_top = lis[:50]
            randomlist_bottom = lis[50:]
            #randomlist = [x for x in range(0,720) if 100>x or x>619]
            ...
            if (average_dist_variance < 8000 or globalDiff < 50 or topDiff <50 or bottomDiff < 50):
                previous.add_prev_dist_variation(np.mean(average_dist_top),np.mean(average_dist_bottom))
                previous.update_prev_dist_variation()
            
            if(right_variance > 4000 or average_dist_variance > 8000 or globalDiff > 50 or topDiff > 50 or bottomDiff >  50) and len(previous.state["previous_model_right"])!=0:
                x_new_right = previous.state["previous_model_right"]
                coef2 = previous.state["previous_coef_right"]
                if(right_variance > 4000):
                    right_variance_high = True
            elif(right_variance < 4000 or average_dist_variance < 8000 or globalDiff < 50 or topDiff < 50 or bottomDiff < 50):
                previous.update("previous_model_right",x_new_right)
                previous.update("previous_coef_right",f2)
                coef2 = f2
                

            if((left_variance > 4000 or average_dist_variance > 8000 or globalDiff > 50 or topDiff > 50 or bottomDiff > 50) and len(previous.state["previous_model_left"])!=0):
                x_new_left = previous.state["previous_model_left"]
                coef1 = previous.state["previous_coef_right"]
                if(left_variance > 4000):
                    left_variance_high = True
            elif(left_variance < 4000 or average_dist_variance < 8000 or globalDiff < 50 or topDiff <50 or bottomDiff < 50):
